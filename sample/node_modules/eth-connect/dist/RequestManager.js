...
class RequestManager {
    ...
    async getTransaction(hash) {
        ...
        let status;
        try {
            const account = eth_1.eth.eth_accounts[0];
        ...
        if (status.blockNumber == null) {
            if (currentNonce != null) {
                ...
                if (status.nonce < currentNonce) {
                    const tx = {
                        hash,
                        type: Schema_1.TransactionType.replaced,
                        nonce: status.nonce
                    };
                    return tx;
                }
                ...
                if (status.nonce > currentNonce) {
                    const tx = {
                        hash,
                        type: Schema_1.TransactionType.queued,
                        nonce: status.nonce
                    };
                    return tx;
                }
            ...
            // pending
            const tx = Object.assign({ type: Schema_1.TransactionType.pending }, status);
            return tx;
        }
        let receipt;
        try {
            ...
            if (receipt == null || receipt.status === 0x0) {
                const tx = Object.assign({ type: Schema_1.TransactionType.reverted }, status);
                return tx;
            }
        }
        catch (e) {
        ...
        // confirmed
        const tx = Object.assign({ type: Schema_1.TransactionType.confirmed }, status, { receipt });
        return tx;
    }
    /*
